...
app = FastAPI(title="Onyx Note Auth Service")
...
async def forgot_password(req: ForgotPasswordReq, db: Session = Depends(get_db)):
    ...
    if not db_user:
        return {"mensagem": "Se o e-mail estiver cadastrado, um link de recuperação foi enviado."}
    ...
    <div style="font-family: 'Times New Roman', serif; max-width: 600px; margin: 0 auto; border: 1px solid #000; padding: 40px; color: #000; background: #fff;">
        ...
        <div style="text-align: center; margin: 40px 0;">
            <a href="{FRONTEND_URL}/?reset_token={reset_token}" 
               style="background: #000; color: #fff; padding: 15px 30px; text-decoration: none; font-size: 16px; border: 1px solid #000;">
        ...
        <p style="font-size: 12px; color: #888; text-align: center;">Onyx Note</p>
    </div>
    """

    if conf is None:
        return {"mensagem": "Se o e-mail estiver cadastrado, um link de recuperação foi enviado.", "debug_link": f"{FRONTEND_URL}/?reset_token={reset_token}"}
    ...
    try:
        ...
        return {"mensagem": "Erro ao enviar e-mail.", "debug_link": f"{FRONTEND_URL}/?reset_token={reset_token}"}

    return {"mensagem": "Link de recuperação enviado para o e-mail."}
...
async def google_callback(code: str, db: Session = Depends(get_db)):
    ...
    async with httpx.AsyncClient() as client:
        ...
        if "error" in token_json:
            raise HTTPException(status_code=400, detail="Erro na autenticação do Google.")
        
        access_token = token_json["access_token"]
        user_info_response = await client.get("https://www.googleapis.com/oauth2/v1/userinfo?alt=json", headers={"Authorization": f"Bearer {access_token}"})
    ...
    token = create_token(google_email)
