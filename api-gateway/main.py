...
app = FastAPI(title="Onyx Note API Gateway")
...
async def proxy_google_login():
    async with httpx.AsyncClient(follow_redirects=False) as client:
        ...
        raise HTTPException(status_code=500, detail="Erro ao contatar servidor de autenticação Google")
...
async def proxy_users_me(token: str = Depends(verificar_token)):
    ...
    async with httpx.AsyncClient() as client:
        response = await client.get("http://user-service:8001/users/me", headers=headers)
        return response.json()
